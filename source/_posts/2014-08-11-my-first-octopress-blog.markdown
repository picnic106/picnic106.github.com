...
description: "使用Octopress和github搭建个人博客" 
...
{% coderay lang:ruby windows的Devkit安装脚本 %}
d:
...
&emsp;&emsp;python没有什么特别之处，按照流程来安装就ok。安装成功后，执行`easy_install pygments`，安装博客高亮模块。我执行命令的时候报错，报"不存在的命令"，后来发现是环境变量的问题，python自己加的path环境变量中包含`D:\Python27\Pythons`没有后面的斜杠，正确应该是`D:\Python27\Pythons\`。

##5.octopress安装

###5.1，用命令行下载octopress源码：  

{% coderay lang:Python 用命令行下载octopress源码 %}
d:
...
进入d:/octopress目录，用文本编辑器打开gemfile文件，将`source “http://rubygems.org/”`，修改为`source “http://ruby.taobao.org/”`。 
 
###5.3，安装octopress依赖项：  
 
{% coderay lang:Python 安装octopress依赖项 %}
d:
cd octopress
gem install bundler
bundle install
...
*说明2*：需要配置ssh,具体配置请看底下的[注意问题1](#ssh "注意问题1")。  
...
{% coderay lang:Python github_pages设置 %}
d:
cd octopress
...
{% coderay lang:Python 将静态站点发布到github %}
rake install  
rake generate  
rake deploy  
{% endcoderay %}  
...
{% coderay lang:YAML _config.yml例子 %}
...
{% coderay lang:Python new_post命令 %}
...
&emsp;&emsp;使用一个文本编辑器(例如win7下使用的是notepad++)打开新文章（`source/_posts`）。看到一块[yaml头信息](http://jekyllrb.com/docs/frontmatter/)设置。  
...
{% coderay lang:YAML YAML头信息categories配置 %}
    # One category
    categories: Sass
...
{% coderay lang:Python 新建网页的rake命令 %}
...
[ssh密钥](https://help.githuarticles/generating-ssh-keys):是一种用来识别受信任计算机的一种方法，而不涉及到密码。下面的步骤将带你产生一个ssh密钥，并将公钥加入到github帐户。  
  
###**步骤1**：  
检查ssh密钥：检查计算机上是否存在ssh密钥,进入windows用户目录，例如我机器上的用户目录是：C:\Users\username。检查这个目录下是否有.ssh文件夹，如果有检查.ssh文件夹下是否有`id_rsa.pub`或者`id_dsa.pub`文件。如果没有进入第二步，如果已经存在了，进入第三步。  

###**步骤2**：  
生成一个ssh密钥：生成一个密钥，需要执行以下命令，确保git/bin目录存在环境变量PATH里，your_email@example.com修改为你github的注册邮箱：  

{% coderay lang:Python 生成ssh密钥 %}
...
###**步骤3**：  
配置github帐号的ssh公钥：使用文本文件打开`id_rsa.pub`文件，复制里面的内容，然后把内容加到github帐号。  
...
{% coderay lang:Python 解决报版本不一致脚本 %}
